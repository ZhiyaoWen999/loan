#!/usr/bin/env python3
"""Build classic cohort×MOB vintage tables from loan DPD snapshots.

This is the "classic" vintage view:
- cohort: disbursement month/week
- MOB: months on book
- metric: dpd30_rate at each MOB and ever_dpd30_rate up to that MOB

Input:
- `loan_dpd_snapshots.csv` (loan_id × snapshot_dt) generated by `generate_mock_data.py`
Output:
- a long table suitable for line charts or heatmaps, optionally segmented by channel/product/score decile.
"""
...
def main() -> None:
    ...
    # cohort = disbursement period; MOB already exists in snapshots (0..N).
    ...
    # Ever DPD30 up to each MOB for the same loan.
    snap["ever_dpd30"] = snap.groupby("loan_id")["dpd30"].cummax()

    # Optional segmentation (e.g. channel/product_type/risk_score_decile).
